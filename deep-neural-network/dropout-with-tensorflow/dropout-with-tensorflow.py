...
hidder_layer = tf.add(tf.matmul(features, weights[0]), biases[0])
hidder_layer = tf.nn.relu(hidder_layer)
hidder_layer = tf.nn.dropout(hidder_layer, keep_prob)

logits = tf.add(tf.matmul(hidder_layer, weights[1]), biases[1])
